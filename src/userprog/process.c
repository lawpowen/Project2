...
static struct thread *child_thread_created = NULL;
static tid_t iter_tid;
static void find_child_tid(struct thread *t, void *aux UNUSED)
{
  if (iter_tid == t->tid)
  {
    child_thread_created = t;
  }
}
...
tid_t process_execute(const char *file_name)
{
  ...
  else
  {
    ...
    thread_foreach(*find_child_tid, NULL);
    list_push_front(&thread_current()->child_process_list, &child_thread_created->child_process_elem);
...
int process_wait(tid_t child_tid UNUSED)
{
  ...
  {
    //do stuff with iter
    struct thread *tt = list_entry(iter, struct thread, child_process_elem);
    if (tt->tid == child_tid)
    {
      t = tt;
      break;
...
bool load(const char *file_name, void (**eip)(void), void **esp)
{
  ...
  char *saveptr;
  file_name_ = strtok_r(file_name_, " ", &saveptr);
